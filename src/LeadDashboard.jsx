...
export default function LeadDashboard({
  ...
  function startEditingDetails() {
    setDetailsDraft({
      ...
      current_provider_scheme_type: selected.currentProviderSchemeType || "",
      current_provider_account_no: selected.currentProviderAccountNo || "",
      current_provider_start_date: selected.currentProviderStartDate || "",
      current_provider_maturity_date: selected.currentProviderMaturityDate || "",
      nationality: selected.nationality || "",
      gender: selected.gender || "",
      number_of_children: selected.numberOfChildren ?? "",
      marital_status: selected.maritalStatus || "",
      spouse_partner_name: selected.spousePartnerName || "",
      country_of_residence: selected.countryOfResidence || "",
  ...
  async function handleSaveDetails() {
    ...
    try {
      ...
      // after every save meant re-clicking "Edit" for each
      // additional change. Now it only exits when you choose to,
  ...
  const filtered = useMemo(() => {
    ...
    rows.sort((a, b) => {
      ...
      if (sortKey === "score") { av = a.score; bv = b.score; }
        ...
        bv = getJurisdiction(jurisdictions, b.jurisdiction).country || "";
      }
            ...
            {!editingDetails ? (
              <dl className="grid grid-cols-2 gap-y-4 gap-x-3 mb-6">
                {[
                  ...
                  ["Scheme type", selected.currentProviderSchemeType || "—"],
                  ["Account No.", selected.currentProviderAccountNo || "—"],
                  ["Scheme start date", formatDate(selected.currentProviderStartDate)],
                  ["Maturity date", formatDate(selected.currentProviderMaturityDate)],
                  ["Nationality", selected.nationality || "—"],
                  ["Gender", selected.gender || "—"],
                  ["Marital status", selected.maritalStatus || "—"],
                  ["Spouse/Partner name", selected.spousePartnerName || "—"],
                  ["Children", selected.numberOfChildren ?? "—"],
              ...
              <div className="mb-6">
                {(() => {
                  ...
                  return (
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      <div><label style={labelStyle}>Scheme type</label><input style={inputStyle} value={detailsDraft.current_provider_scheme_type} onChange={(e) => dset("current_provider_scheme_type", e.target.value)} placeholder="e.g. SIPP, ISA, Pension" /></div>
                      <div><label style={labelStyle}>Account No.</label><input style={inputStyle} value={detailsDraft.current_provider_account_no} onChange={(e) => dset("current_provider_account_no", e.target.value)} /></div>
                      <div><label style={labelStyle}>Scheme start date</label><input type="date" style={inputStyle} value={detailsDraft.current_provider_start_date} onChange={(e) => dset("current_provider_start_date", e.target.value)} /></div>
                      <div><label style={labelStyle}>Maturity date</label><input type="date" style={inputStyle} value={detailsDraft.current_provider_maturity_date} onChange={(e) => dset("current_provider_maturity_date", e.target.value)} /></div>
                      <div><label style={labelStyle}>Trustee</label><input style={inputStyle} value={detailsDraft.trustee} onChange={(e) => dset("trustee", e.target.value)} /></div>
                      <div><label style={labelStyle}>Nationality</label><input style={inputStyle} value={detailsDraft.nationality} onChange={(e) => dset("nationality", e.target.value)} /></div>
                      <div>
                        <label style={labelStyle}>Gender</label>
                        <select style={inputStyle} value={detailsDraft.gender} onChange={(e) => dset("gender", e.target.value)}>
                          <option value="">—</option>
                      ...
                      <div>
                        ...
                        <select style={inputStyle} value={detailsDraft.marital_status} onChange={(e) => dset("marital_status", e.target.value)}>
                          <option value="">—</option>
                      ...
                      <div><label style={labelStyle}>Spouse/Partner name</label><input style={inputStyle} value={detailsDraft.spouse_partner_name} onChange={(e) => dset("spouse_partner_name", e.target.value)} /></div>
                      <div><label style={labelStyle}>Children</label><input type="number" min="0" style={inputStyle} value={detailsDraft.number_of_children} onChange={(e) => dset("number_of_children", e.target.value)} /></div>
                ...
                {justSaved && (
                  <div style={{ fontSize: 12, color: TOKENS.riskLow, marginTop: 8, textAlign: "center" }}>
                    Saved — you can keep editing, or close this panel whenever you're done.
                  </div>
                )}
              </div>
            )}
